...
// ══ IMAGE COMPRESSION ══
// Resizes to max 1600px wide & converts to JPEG 75% quality
// Reduces ~10MB photos to ~200-400KB
function compressImage(dataUrl, callback, maxW, quality) {
  maxW = maxW || 1600;
  ...
  img.onload = function() {
    ...
    var origKB = Math.round(dataUrl.length * 0.75 / 1024);
    var newKB = Math.round(compressed.length * 0.75 / 1024);
    console.log('Image compressed: ' + origKB + 'KB → ' + newKB + 'KB (' + w + 'x' + h + ')');
    callback(compressed);
  };
  img.src = dataUrl;
}
...
angleFileInput.addEventListener('change',function(e){
  ...
  var r=new FileReader();r.onload=function(ev){
    compressImage(ev.target.result, function(compressed){
      ...
      if(pendingAngleUpload.step===1){a.step1=compressed;a.s1n=f.name}
      else{a.step2=compressed;a.s2n=f.name}
...
patternFileInput.addEventListener('change',function(e){var f=e.target.files[0];if(!f)return;var r=new FileReader();r.onload=function(ev){compressImage(ev.target.result,function(compressed){patterns.push({data:compressed,fn:f.name,pn:'',cl:''});activePattern=patterns.length-1;renderPatterns();applyView();animateTo(100)})};r.readAsDataURL(f);patternFileInput.value=''});

// ══ EXPORT ══
function estimateSize(){
  var totalBytes=0;
  angles.forEach(function(a){if(a.step1)totalBytes+=a.step1.length*0.75;if(a.step2)totalBytes+=a.step2.length*0.75});
  patterns.forEach(function(p){totalBytes+=p.data.length*0.75});
  var mb=(totalBytes/1024/1024).toFixed(1);
  $('exportFilename').textContent+='　（推定 '+mb+'MB）';
}
