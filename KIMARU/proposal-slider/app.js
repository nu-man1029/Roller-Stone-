...
var GITHUB_CLIENTS_BASE='KIMARU/proposal-slider/clients';
...
$('btnUploadGitHub').addEventListener('click',async function(){
  ...
  try{
    var info={client:fieldClient.value.trim(),site:fieldSite.value.trim(),date:fieldDate.value,note:fieldNote.value.trim()};
    var html=buildExport(info);
    var path=GITHUB_CLIENTS_BASE+'/'+folder+'/index.html';
    var msg='Add proposal: '+folder+(info.client?' ('+info.client+')':'');
    await pushToGitHub(token,path,html,msg);

    var pageUrl='https://nu-man1029.github.io/Roller-Stone-/KIMARU/proposal-slider/clients/'+folder+'/';
